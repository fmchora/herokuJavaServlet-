...
// The factory instance is re-useable and thread safe.
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("Tweet:" + tweets.get(0).getText());
            out.println("key:" + twitter.getConfiguration().getOAuthConsumerKey());
            out.println("secret: " + twitter.getConfiguration().getOAuthConsumerSecret());
            out.println("Url: " + tweets.get(0).getCreatedAt() );
            
            request.setAttribute("tweets", tweets);
            request.getRequestDispatcher("twitter.jsp").forward(request, response);

            //for (Status status : tweets) {
              //  out.println("<p> " + status.getUser().getName() + ":" + status.getText() + "</p>");
            //}
